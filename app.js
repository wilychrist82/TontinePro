    ...
    if (typeof DataService !== 'undefined') {
        ...
        if (window.SupabaseService && window.SupabaseService.fetchPaymentsForReports) {
            ...
            if (payments && payments.length > 0) {
                ...
                state.stats.totalMembersCount = state.activeTontines.reduce((acc, t) => acc + (parseInt(t.members.split('/')[0]) || 0), 0);
            }
        }
    }
}
...
function renderTontinesTable() {
    ...
    state.activeTontines.forEach(t => {
        ...
        tr.innerHTML = `
            ...
            <td><span class="badge-status ${t.status === 'En cours' ? 'badge-green' : 'badge-yellow'}">${t.status}</span></td>
            <td>
        ...
        <tr>
            ...
            <td>
                <div class="action-dropdown" style="position:relative;">
                    <button class="btn-more" onclick="toggleTxDropdown('${tx.id}')">&#8942; Options</button>
