...
- **Axe 5 (Terminé — Session Actuelle)** :
  ...
  5. **Garantie Anti-Truquage & Certification** : Après le 1er tirage, le bouton est masqué et remplacé par un sceau `🛡️ Tirage Clôturé & Certifié` avec code unique (ex: `#CERT-8F39`) et horodatage. Bouton `💬 Notifier WhatsApp` pour envoyer le certificat officiel au groupe. Si le gestionnaire force une réinitialisation (`resetTontineDraw`), une alerte de sécurité avertit le groupe.
  6. **Correction Bug Permissions Admin/Gestionnaire** : Ajout explicite des actions `'edit_tontine'` et `'delete_tontine'` dans la table `PERMISSIONS` pour les rôles `admin`, `administrateur` et `gestionnaire` dans `app_v5.js`. Plus de message d'erreur intempestif lors de la modification ou suppression d'une tontine en mode Administrateur.
  7. **Délégation de Pouvoir / Intérim (`🤝 Déléguer Admin`)** : Le gestionnaire peut à tout moment donner la main à un membre de confiance depuis l'onglet `Administration` en cliquant sur `🤝 Déléguer Admin`. Ce membre reçoit immédiatement le rôle `🛡 Administrateur` avec 100% des droits (tirages, paiements, clôtures, invitations). Au retour du gestionnaire, il suffit de cliquer sur `🚫 Retirer droits` pour restaurer le statut de simple membre. Un encadré explicatif a été ajouté au sommet du tableau de bord administrateur.
  8. **Option B : Gestion Automatique des Pénalités de Retard (`#tontine-penalties-section`)** : Intégration d'un module interactif de suivi des pénalités dans la modale de détails de la tontine (`#tontine-details-modal`) et dans le modal de validation de paiement (`#validate-payment-modal`). Détection automatique des membres en retard (ex: >3 jours de retard = +2 000 FCFA de pénalité), calcul et alimentation d'une **Caisse de Réserve & Pénalités** cumulative. Bouton de régularisation en 1 clic (`resolveMemberPenalty`) avec émission de reçu incluant la pénalité.
